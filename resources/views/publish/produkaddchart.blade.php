...
<a class="right carousel-control" href="#myCarousel" role="button" data-slide="next">
 ...
 <!--End Featured products Div-->
    ...
    <div class="container">
        <h2 class="title-div wow slideInLeft" data-wow-duration="1s" data-wow-delay="0s" data-wow-offset="10">Produk</h2>
        <div class="products">
            <div class="row">
				...
				<div class="wrapper row">
					<div class="preview col-md-6">
						...
						<div class="preview-pic tab-content">
						  <div class="tab-pane active" id="pic-1"><img src="{{ asset('storage/produk/'.$itemproduk->path.'') }}" width="400" height="200"/></div>
						</div>
						
					</div>
					<div class="details col-md-6">
                        <form method="POST" action="{{ url('publish/product/troly') }}" enctype="multipart/form-data">
                        @csrf
                            <h3 class="product-title"><font color="brown">{{ $itemproduk->nama }}</font></h3>
                            
                            <p class="product-description">{{ $itemproduk->deskripsi }}.</p>
                            <h4 class="price">current price: <span>{{ number_format($itemproduk->harga) }}</span></h4>
                            <p class="card-text"><font color="red">Stok : {{ $itemproduk->stok }}</font></p>
                            <div class="form-group">
                                <input type="number" name="qty" class="form-control" id="number" placeholder="qty beli">
                                 
                            </div>
                            <div class="action">
                                <a href="{{ url('/products') }}" class="btn btn-warning">Product</a>
                                <input type="hidden" value="{{ Auth::user()->id }}" name="user_id">
                                <input type="hidden" value="{{ $itemproduk->id }}" name="idproduk">
                                <input type="hidden" value="{{ $itemproduk->harga }}" name="harga">
                                @if($itemproduk->stok > '0')
                                <button class="btn btn-success btn-submit">Confirm</button>
                                @else
                                <a href="" class="btn btn-danger" DISABLED>Stock empty</a>
                                @endif
            ...
            <p style="text-align:right;">
                            <a href="{{ url('/mywish') }}" class="btn btn-primary">My Wishlist</a>
                        </p>
            <div class="clear"></div>
